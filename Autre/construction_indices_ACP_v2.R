...
# Variables standardisées (mean=0, std=1), 306 zones d'emploi.
# Aucune valeur manquante.
#
# NOTA : prcomp() est appelé avec center = FALSE et scale. = FALSE car les
# variables sont déjà standardisées (mean=0, std=1) dans data_retraited.xlsx.
# Ne pas modifier ces paramètres sans restandardiser les données en amont.
# =============================================================================
...
# -----------------------------------------------------------------------------
# 2. Résolution des noms de colonnes à encodage variable
# -----------------------------------------------------------------------------
# Certains noms de colonnes contiennent des apostrophes ou caractères dont
# l'encodage exact peut différer selon l'OS ou l'éditeur.
# On les résout dynamiquement via grep() pour garantir la correspondance.

var_multi_etr <- colnames(data_retraited)[
  grep("multinationales.*trang", colnames(data_retraited), ignore.case = TRUE)
]
var_multi     <- colnames(data_retraited)[
  grep("multinationales.dans", colnames(data_retraited), ignore.case = TRUE)
]
var_znieff    <- colnames(data_retraited)[
  grep("ZNIEFF", colnames(data_retraited), ignore.case = TRUE)
]

# Vérification
cat("Variables résolues dynamiquement :\n")
cat(" -", var_multi_etr, "\n")
cat(" -", var_multi, "\n")
cat(" -", var_znieff, "\n\n")

# -----------------------------------------------------------------------------
# 3. Définition des dimensions thématiques
# -----------------------------------------------------------------------------
...
dimensions <- list(
  ...
  dynamisme_economique_emploi = c(
    ...
    var_multi_etr,   # résolu dynamiquement
    var_multi,       # résolu dynamiquement
    "Taux.de.pauvreté",
  ...
  environnement_risques = c(
    ...
    "Zones humides",
    "Surfaces en eau",
    var_znieff,      # résolu dynamiquement
    "indice_DDRM",
    "indice_catnat"
  ),
...
#
# NOTE sur fiscalite_ressources_publiques et logement_cadre_residentiel :
# Ces deux dimensions présentent une corrélation négative avec nombre_creations
# (respectivement r = -0.567 et r = -0.458). Ceci n'est pas un artefact
# technique mais un résultat substantiel : le PC1 de ces dimensions capture
# une intensité des charges publiques / un profil résidentiel stable, qui
# caractérise des territoires peu dynamiques sur le plan entrepreneurial.
# Ce résultat est documenté et assumé comme limite connue de l'indice.
# Les variables d'orientation ne sont pas modifiées a posteriori pour éviter
# tout biais de sélection (OCDE/JRC 2008, ch. 6).
...
# -----------------------------------------------------------------------------
# 5. Calcul des scores dimensionnels
# -----------------------------------------------------------------------------
...
# -----------------------------------------------------------------------------
# 6. Indice global — moyenne simple des 9 scores dimensionnels
# -----------------------------------------------------------------------------
...
#
# Les dimensions à corrélation négative avec nombre_creations
# (fiscalite_ressources_publiques, logement_cadre_residentiel) sont conservées
# dans l'indice global. Les retirer a posteriori constituerait une sélection
# guidée par les résultats, incompatible avec le cadre OCDE/JRC 2008.
...
r_global   <- cor(score_global_norm, data_retraited$nombre_creations,
                  use = "complete.obs")
rho_global <- cor(score_global_norm, data_retraited$nombre_creations,
                  method = "spearman", use = "complete.obs")

cat(sprintf("\n=== Indice global (moyenne simple des 9 scores) ===\n"))
cat(sprintf("Corrélation de Pearson  ~ créations : r   = %.3f\n", r_global))
cat(sprintf("Corrélation de Spearman ~ créations : rho = %.3f\n\n", rho_global))
...
# -----------------------------------------------------------------------------
# 8. Diagnostic complémentaire — corrélations individuelles par dimension
# -----------------------------------------------------------------------------
# Produit un tableau des corrélations de chaque variable source avec
# nombre_creations, pour chaque dimension. Utile pour la discussion des
# limites méthodologiques (dimensions à r négatif).

cat("\n=== Corrélations individuelles variables / nombre_creations ===\n")
for (dim in names(resultats)) {
  vars_dim <- resultats[[dim]]$vars_ok
  cor_dim  <- sapply(vars_dim, function(v) {
    cor(data_retraited[[v]], data_retraited$nombre_creations, use = "complete.obs")
  })
  cor_dim_sorted <- sort(cor_dim)
  cat(sprintf("\n[%s]\n", dim))
  for (i in seq_along(cor_dim_sorted)) {
    cat(sprintf("  %+.3f  %s\n", cor_dim_sorted[i], names(cor_dim_sorted)[i]))
  }
}

# -----------------------------------------------------------------------------
# 9. Graphiques de diagnostic
# -----------------------------------------------------------------------------
...
# --- 9a. Corrélations des scores avec nombre_creations ---
...
cat("\nFigure : figures/correlations_scores_creations.png\n")

# --- 9b. Variance expliquée par le PC1 ---
...
# --- 9d. Distribution de l'indice global ---
...
# --- 9e. Indice global vs nombre_creations ---
...
p_scatter <- ggplot(data_scores, aes(x = score_global, y = nombre_creations)) +
  ...
  annotate("text",
           ...
           label = sprintf("r = %.3f  |  rho = %.3f", r_global, rho_global),
           size  = 4, color = "gray20") +
  labs(
...
cat(sprintf("Indice global : r = %.3f (Pearson) | rho = %.3f (Spearman)\n",
            r_global, rho_global))

# -----------------------------------------------------------------------------
# 10. Analyse de robustesse — OCDE/JRC 2008, ch. 8
# -----------------------------------------------------------------------------

cat("\n=== Analyse de robustesse (OCDE/JRC 2008, ch. 8) ===\n")

# Poids scénario A : moyenne simple (méthode de base)
poids_A <- rep(1/9, 9)
names(poids_A) <- cols_dim

# Poids scénario B : proportionnel à la variance expliquée PC1
var_exp <- sapply(names(resultats), function(d) resultats[[d]]$var_exp_pc1)
poids_B <- var_exp / sum(var_exp)

# Poids scénario C : proportionnel à la corrélation absolue avec créations
# (présenté comme informatif uniquement — ne pas utiliser comme méthode principale)
r_abs <- abs(sapply(names(resultats), function(d) resultats[[d]]$r_creations))
poids_C <- r_abs / sum(r_abs)

# Calcul des indices sous chaque scénario
calc_indice <- function(scores_mat, poids) {
  brut <- as.numeric(scores_mat %*% poids)
  (brut - min(brut)) / (max(brut) - min(brut)) * 100
}

score_A <- calc_indice(scores_matrix, poids_A)  # identique à score_global
score_B <- calc_indice(scores_matrix, poids_B)
score_C <- calc_indice(scores_matrix, poids_C)

# Corrélations de Spearman entre classements
rho_AB <- cor(rank(score_A), rank(score_B), method = "spearman")
rho_AC <- cor(rank(score_A), rank(score_C), method = "spearman")
rho_BC <- cor(rank(score_B), rank(score_C), method = "spearman")

cat(sprintf("Corrélation de rang Spearman entre scénarios :\n"))
cat(sprintf("  A (moyenne simple) vs B (pondéré PC1)      : rho = %.3f\n", rho_AB))
cat(sprintf("  A (moyenne simple) vs C (pondéré |r|)      : rho = %.3f\n", rho_AC))
cat(sprintf("  B (pondéré PC1)    vs C (pondéré |r|)      : rho = %.3f\n", rho_BC))

# Zones dont le rang change le plus entre A et B
rang_A <- rank(-score_A)
rang_B <- rank(-score_B)
rang_C <- rank(-score_C)

delta_AB <- abs(rang_A - rang_B)
delta_AC <- abs(rang_A - rang_C)

robustesse_df <- data.frame(
  ZE       = data_retraited$LIBZE2020,
  rang_A   = rang_A,
  rang_B   = rang_B,
  rang_C   = rang_C,
  delta_AB = delta_AB,
...
cat("\nTop 10 zones avec le plus grand écart de rang entre scénarios A et B :\n")
print(robustesse_df[order(-delta_AB), c("ZE", "rang_A", "rang_B", "delta_AB")][1:10, ],
      row.names = FALSE)

# Poids sous chaque scénario (pour documentation)
cat("\nPoids par dimension sous chaque scénario :\n")
poids_df <- data.frame(
  dimension = names(resultats),
  poids_A   = round(poids_A * 100, 1),
  poids_B   = round(poids_B * 100, 1),
  poids_C   = round(poids_C * 100, 1)
)
print(poids_df, row.names = FALSE)

# Export résultats de robustesse
data_scores$score_B <- score_B
data_scores$score_C <- score_C
